...
def parse_args():
    ...
    parser.add_argument('--drop_out_keep', nargs='?', default='[1.0,1.0,1.0]',
...
class DeepCross:
    def __init__(self, features_M, pretrain_flag, save_file, hidden_factor, loss_type, epoch, batch_size, learning_rate,
                 l2_reg, drop_out_keep, optimizer_type, batch_norm, verbose, num_field, cross_layer_num,
            ...
            for epoch in range(self.epoch):
                ...
                for i in range(total_batch):
                    ...
                    feed_dict = {self.train_features: batch_xs['X'], self.train_labels: batch_xs['Y'],
                                 self.dropout_keep: self.drop_out_keep, self.train_phase: True}
    ...
    def initialize_variables(self):
        ...
        if self.pretrain_flag > 0:
            ...
            self.total_size = self.num_field * self.hidden_factor
            ...
            self.weights['deep_bias_0'] = tf.Variable(
                np.random.normal(loc=0, scale=glorot, size=(1, self.deep_layers[0])), dtype=np.float32
            )

            for i in range(1, num_layer):
                glorot = np.sqrt(2.0 / (self.deep_layers[i - 1] + self.deep_layers[i]))
                self.weights["deep_layer_%d" % i] = tf.Variable(
                    np.random.normal(loc=0, scale=glorot, size=(self.deep_layers[i - 1], self.deep_layers[i])),
                    dtype=np.float32)  # layers[i-1] * layers[i]
                self.weights["deep_bias_%d" % i] = tf.Variable(
                    np.random.normal(loc=0, scale=glorot, size=(1, self.deep_layers[i])),
                    dtype=np.float32)  # 1 * layer[i]

            for i in range(self.cross_layer_num):
                self.weights["cross_layer_%d" % i] = tf.Variable(
                    np.random.normal(loc=0, scale=glorot, size=(self.total_size, 1)),
                    dtype=np.float32)
                self.weights["cross_bias_%d" % i] = tf.Variable(
                    np.random.normal(loc=0, scale=glorot, size=(self.total_size, 1)),
                    dtype=np.float32)  # 1 * layer[i]
    ...
    def create_inference_DeepCross(self):

        # Embeddings
        # None * M * K
        self.embeddings = tf.nn.embedding_lookup(self.weights['feature_embeddings'], self.train_features)
        # None * (M * K)
        self.x0 =  tf.reshape(self.embeddings,[-1, self.num_field * self.hidden_factor])

        # deep part
        self.y_deep = tf.nn.dropout(self.x0, self.dropout_keep[0])

        for i in range(0, len(self.deep_layers)):
            # None * (M * K) MAT (M * K) * layer[i] -> None * layer[i]
        ...
        for l in range(self.cross_layer_num):
            # None * (M * K) * 1 MAT None * 1 * (M * K) -> None * (M * K) * (M * K)
            # None * (M * K) * (M * K) DOT (M * K) * 1 -> None * (M * K) * 1
            # None * (M * K) * 1 + (M * K) * 1 + None * (M * K) * 1 -> None * (M * K) * 1
        ...
        # None * (M * K + layer[i])  *  (M * K + layer[i]) * 1 -> None * 1
    ...
    def build_graph(self):
        ...
        self.create_inference_DeepCross()
        self.create_loss()
    ...
    def evaluate(self, data):  # evaluate the results for an input set
        ...
        while len(batch_xs['X']) > 0:
            ...
            feed_dict = {self.train_features: batch_xs['X'], self.train_labels: [[y] for y in batch_xs['Y']],
                         self.dropout_keep: list(1.0 for i in range(len(self.drop_out_keep))), self.train_phase: False}
...
if __name__ == '__main__':
    ...
    if args.verbose > 0:
        logging.info('DeepCross:dataset=%s,pretrain=%d,save_file=%s,hidden_factor=%d,loss_type=%s,#epoch=%d, batch=%d,lr=%.4f,'
                     'l2_reg=%.1e,drop_out_keep=%s,optimizer=%s, batch_norm=%d,verbose=%s,num_field=%d,cross_layer_num=%d,deep_layers=%s,'
                   ...
                   args.batch_size, args.lr, args.l2_reg, args.drop_out_keep, args.optimizer, args.batch_norm,args.verbose,
                      args.num_field, args.cross_layer_num, args.deep_layers,args.activation))
    ...
    model = DeepCross(data.features_M, args.pretrain, save_file, args.hidden_factor, args.loss_type, args.epoch,
                   args.batch_size, args.lr, args.l2_reg, eval(args.drop_out_keep), args.optimizer, args.batch_norm,args.verbose,
                      args.num_field, args.cross_layer_num, eval(args.deep_layers),args.activation)
    model.train(data)
